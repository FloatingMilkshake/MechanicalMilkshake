...
internal class CommandTasks
{
    ...
    private static async Task PopulateApplicationCommandListAsync()
    {
        ...
        applicationCommands.AddRange(await Setup.State.Discord.Client.GetGuildApplicationCommandsAsync(Setup.State.Discord.HomeServer.Id) as List<DiscordApplicationCommand> ?? []);
    ...
    private static async Task RunApplicationCommandRegistrationWatchdogAsync()
    {
        ...
        if (Setup.State.Discord.ApplicationCommands.Count == 0)
        {
            ...
            {
                await Setup.State.Discord.Channels.Home.SendMessageAsync(string.Join(" ", Setup.State.Discord.Client.CurrentApplication.Owners.Select(o => o.Mention))
