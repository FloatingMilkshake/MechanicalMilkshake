...
public class HastebinHelpers
{
    ...
    {
        if (Program.ConfigJson.HastebinUrl is null) return "Failed to upload; Hastebin URL missing from config.json";
        
        var client = Program.HttpClient;
        var response =
            await client.PostAsync($"{Program.ConfigJson.HastebinUrl}/documents", new StringContent(text));
        ...
        return $"{Program.ConfigJson.HastebinUrl}/{responseJson["key"]}";
    }
}