...
{
    [SlashCommandGroup("httpcat", "Get an HTTP cat image!")]
    private class HttpCatCmds
    {
        ...
        [SlashCommand("random", "Get a random http.cat image!")]
        public static async Task HttpCatRandom(InteractionContext ctx)
        {
            await ctx.CreateResponseAsync(InteractionResponseType.DeferredChannelMessageWithSource);

            var random = new Random();
            var code = HttpCodes[random.Next(HttpCodes.Count)];
        ...
        [SlashCommand("code", "Get the http.cat image for an HTTP code.")]
        public static async Task HttpCatCode(InteractionContext ctx,
