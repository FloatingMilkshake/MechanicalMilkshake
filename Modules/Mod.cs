namespace MechanicalMilkshake.Modules
{
    ...
    public class Mod : ApplicationCommandModule
    {
        ...
        public async Task Clear(InteractionContext ctx,
            ...
            [Option("ignore_me", "Optionally filter the deletion to only messages not sent by you.")] bool ignoreMe = false,
        ...
        )
        {
            ...
            if (count == 0 && upTo == "" && user == default && ignoreMe == false && match == "" && botsOnly == false && humansOnly == false && attachmentsOnly == false && linksOnly == false)
            {
                await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"You must provide at least one argument! I need to know which messages to delete.").AsEphemeral(true));
                return;
            }

            if (count == 0 && upTo == "")
            {
                await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"I need to know how many messages to delete! Please provide a value for `count` or `up_to`.").AsEphemeral(true));
                return;
            }
            ...
            if (count < 0)
            {
                await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"I can't delete a negative number of messages! Try setting `count` to a positive number.").AsEphemeral(true));
                return;
            }

            if (count >= 1000)
            {
                await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"Deleting that many messages poses a risk of something disastrous happening, so I'm refusing your request, sorry.").AsEphemeral(true));
                return;
            }
            ...
            if (upTo != "" && count != 0)
            {
                await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"You can't provide both a count of messages and a message to delete up to! Please only provide one of the two arguments.").AsEphemeral(true));
                return;
            }
            ...
            else
            {
                ...
                if (!upTo.Contains("discord.com"))
                {
                    ...
                    {
                        await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"That doesn't look like a valid message ID or link! Please try again."));
                        return;
                    }
                }
                else
                {
                    ...
                    {
                        await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"Please provide a valid link to a message in this channel!").AsEphemeral(true));
                        return;
                    }
                }
            ...
            if (botsOnly)
            {
                if (humansOnly)
                {
                    await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"You can't use `bots_only` and `humans_only` together! Pick one or the other please.").AsEphemeral(true));
                    return;
                }
            ...
            if (attachmentsOnly)
            {
                if (linksOnly)
                {
                    await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"You can't use `images_only` and `links_only` together! Pick one or the other please.").AsEphemeral(true));
                    return;
                }
            ...
            {
                await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"All of the messages to delete are older than 2 weeks, so I can't delete them!").AsEphemeral(true));
                return;
            }
            ...
            if (messagesToClear.Count >= 50)
            {
                ...
                DiscordMessage confirmationMessage = await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"messages. Are you sure?").AddComponents(confirmButton).AsEphemeral(true));
            ...
            else
            {
                if (messagesToClear.Count >= 1)
                {
                    ...
                    if (skipped)
                    {
                        await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"Cleared **{messagesToClear.Count}** messages from {ctx.Channel.Mention}!\nSome messages were not deleted because they are older than 2 weeks.").AsEphemeral(true));
                    }
                    else
                    {
                        await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"Cleared **{messagesToClear.Count}** messages from {ctx.Channel.Mention}!").AsEphemeral(true));
                    }
                }
                else
                {
                    await ctx.FollowUpAsync(new DiscordFollowupMessageBuilder().WithContent($"There were no messages that matched all of the arguments you provided! Nothing to do."));
                }
            }
        }
